...
function class.new(x)
	...
	function chunk:blockToXY(index)
		index = index - 1
		return (index % world.chunkwidth) + (chunk.x * world.chunkwidth), -(math.floor(index / world.chunkwidth) - world.chunkheight / 2)
	end
	...
	function chunk:draw()
		...
		for i,v in next, (self.blocks) do
			if v > 1 then
				...
				render:drawGraphic(blocks[v].sprite, x, y + world.chunkheight / 2, 0, nil, blocksize / 2, blocksize / 2)
			end
		end
	end
